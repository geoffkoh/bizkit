...
- **Workflow store**: bizkit's own database (SQLite in dev, Postgres in
  ...
  (spec D45): `bizkit store upgrade` applies them, creating a fresh store
...
- **The store schema changes only through a migration** (spec D45).
...
- Store models touched? An Alembic revision accompanies the change, the
  ...
  test still proves existing rows survive `upgrade head` (D45).
