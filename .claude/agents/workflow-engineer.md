...
- `src/bizkit/store/` — SQLAlchemy models, repositories, engine factory, the optional store-backed config adapters (`StoreAccessPolicy`, `StoreTableRegistry`), and the Alembic migration chain (`alembic.ini`, `migrations/env.py`, `migrations/versions/`, `schema.py` driver — spec D45)
...
- Store changes get round-trip tests against in-memory SQLite, **plus a migration** (D45): autogenerate it, then read the script — autogenerate renders a rename as drop + create, which destroys the column's data. The retention test (`tests/test_store/test_migrations.py`) must still show populated rows surviving `upgrade head`.
- Run `uv run pytest`, `uv run ruff check .`, `uv run mypy .` before declaring done.

## Anti-patterns to reject on sight

- Changing `store/models.py` without a migration, or treating `create_all` as an upgrade path — it silently no-ops on an existing table (spec D45).
