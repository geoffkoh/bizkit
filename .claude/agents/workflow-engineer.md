...
- `src/bizkit/store/` — SQLAlchemy models, repositories, engine factory, the optional store-backed config adapters (`StoreAccessPolicy`, `StoreTableRegistry`), and the Alembic migration chain (`alembic.ini`, `migrations/env.py`, `migrations/versions/`, `schema.py` driver — spec D46)
...
12. The store schema changes **only through an Alembic revision** (spec D46). `metadata.create_all` is not an upgrade path — it skips existing tables, so a changed model silently leaves a populated store behind. Migrations are forward-only (rollback is restore-from-backup), follow expand/contract (additive and N−1-compatible now, the contracting drop a release later, renames as add + backfill + drop), and **never rewrite audit rows** — D35's append-only guarantee holds through upgrades, so audit shape changes are added columns, not transformed history. The app verifies the revision at startup and refuses to start on a mismatch; it never migrates itself.
...
- Store changes get round-trip tests against in-memory SQLite, **plus a migration** (D46): autogenerate it, then read the script — autogenerate renders a rename as drop + create, which destroys the column's data. The retention test (`tests/test_store/test_migrations.py`) must still show populated rows surviving `upgrade head`.
- Run `uv run pytest`, `uv run ruff check .`, `uv run mypy .` before declaring done.

## Anti-patterns to reject on sight

- Changing `store/models.py` without a migration, or treating `create_all` as an upgrade path — it silently no-ops on an existing table (spec D46).
- A migration that rewrites or deletes audit rows, drops a column in the same release that stopped using it, or renames in place instead of add + backfill + drop.
- Migrating from application startup, or requiring DDL rights on the runtime store credential.
