...
12. **One command palette, one toast system** (§3, §6): the palette is a single component reading from the same tables/changesets queries the sidebar and queue already use via TanStack Query — never a parallel fetch or a hardcoded result list. Toasts are a single app-level stack (top-right, `shadow-md`) driven by a shared `showToast(message, variant)` call from mutation `onSuccess`/`onError` handlers — never a bespoke notification per screen, and never a toast standing in for the inline 403/validation message a control still needs.
13. **Identity is display-only, sourced from `GET /api/v1/me`** (spec D42): the dev `UserPicker` (editable, `localStorage`-backed) renders *only* when `/api/v1/me` reports `auth.provider: none`. Every other provider replaces it with a "Sign in" redirect (`oidc`) or a login form (`ldap`), and the topbar becomes read-only — never a client-editable identity outside dev mode. The SPA never holds a token under `oidc`/`ldap` (backend-mediated session, D42) — don't add token storage to the frontend for these modes.
