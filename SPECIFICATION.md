...
checker) so authorization is exercised out of the box.
...
- ✅ `backends/` — base with lazy-driver discipline (D3), registry with
  ...
  dialect stubs with quirk docstrings. Introspection/read/dry-run/apply
  mechanics raise `NotImplementedError` pending the backend milestone.
...
- ✅ `services/` — `WorkflowService` fully operational
  ...
  audit event per transition); `CommentService`; `ValidationService`
  (runs rule sets — rule `evaluate()` bodies pending);
...
- ✅ `api/` — `create_app` factory; `/api/health` + `/api/ready`
  ...
  audit trail; 501 placeholders for validate/import; SPA mount.
- ✅ `cli/` — `init-store [--seed-sample]` (writes sample workspace +
  ...
  self-approval posture + secret check), `config schema`; explicit
  stubs for show/submit/review/apply/validate/comment/import.
- ✅ `tests/` — 177 passing: full transition matrix, four-eyes strict +
  ...
  Gates: pytest, ruff check/format, mypy strict all clean; wheel builds
  with `py.typed` + `api/static/`.
...
- ⬜ Pending milestones:
  backend mechanics for the 7 enterprise targets
